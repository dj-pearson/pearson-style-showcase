...
| Variable                           | Type   | Required        | Used By                                          | Description                                                                                                                                                                                                                       |
| ---------------------------------- | ------ | --------------- | ------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `CLOUDFLARE_PAGES_DEPLOY_HOOK_URL` | Secret | **Recommended** | `generate-ai-article`, `amazon-article-pipeline` | Cloudflare Pages deploy hook. Posting to it rebuilds the site so a new article gets prerendered HTML, a sitemap entry and an RSS item. Without it, automated posts stay invisible to non-JS crawlers until the next push to main. |
| `INDEXNOW_KEY`                     | Public | Optional        | `maintenance-runner`                             | Overrides the IndexNow key. Must match a `public/<key>.txt` file containing the key. Defaults to the key committed there.                                                                                                         |
...
| `amazon-article-pipeline`     | `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `LOVABLE_API_KEY`      | `SERPAPI_KEY`, `GOOGLE_SEARCH_*`, `CLOUDFLARE_PAGES_DEPLOY_HOOK_URL` |
...
| `generate-ai-article`         | `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `LOVABLE_API_KEY`      | `CLOUDFLARE_PAGES_DEPLOY_HOOK_URL`                                   |
