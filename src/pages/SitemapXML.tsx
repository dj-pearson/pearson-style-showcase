      ...
      try {
        ...
        const [articlesRes, projectsRes, aiToolsRes] = await Promise.all([
          supabase
            .from('articles')
            .select(
              'slug, updated_at, created_at, image_url, title, category, tags, featured, author'
            )
            .eq('published', true)
...
${allUrls
  ...
  // Return XML content
  return <div style={{ whiteSpace: 'pre-wrap', fontFamily: 'monospace' }}>{sitemapXml}</div>;
};
